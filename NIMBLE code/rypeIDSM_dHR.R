rypeIDSM <- nimbleCode({
  ...
  for(t in 1:N_years){
    ...
    # TODO: This most likely needs adjusting for the hazard rate funtion (probably not correct at the moment)
  ...
  for (i in 1:N_obs){ 
    # LIKELIHOOD
    # using nimbleDistance::dHR
    y[i] ~ dHR(b = b, sigma = sigma[Year_obs[i]], Xmax = W, point = 0)
  }

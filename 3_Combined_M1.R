...
for (j in 1:N_sites){
  ...
  N_line_year[j,1] ~ dbin(p[1], N_exp[j,1])
}
#####################################################
## Model for survival; 
...
for(j in 1:N_sites){
  for(t in 2:N_years){
    ...
    N_exp[j,t] ~ dpois(Density[j,t]*L[j,t]*W*2)
...
", fill=TRUE,file="Combined_M1.bugs")
...
L <- matrix(ncol=N_years, nrow=N_sites, rep(1000, each=N_sites*N_years))
A <- colSums(L)*W*2

N_line_year <- matrix(ncol=N_years, nrow=N_sites, rpois(100, 100))

jags.dat2 <- list(R_obs=R_obs, y=y, N_years=N_years, W=W, 
...
out2 <- jagsUI::jags(jags.dat2, inits=inits2, params, model.file="Combined_M1.bugs",
